...
export default function AuthorProfileClient({ initialAuthor }: { initialAuthor: Author }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const isOwnProfileForTracking = userRole === 'author' && authorProfile?.id === authorId
      const shouldTrackView = !isOwnProfileForTracking && !sessionStorage.getItem(viewStorageKey)
      if (shouldTrackView) {
        sessionStorage.setItem(viewStorageKey, '1')
        void supabase.from('profile_views').insert([{ author_id: authorId, viewer_id: userId || null }]).then(({ error }) => {
          if (error) {
    ...
    init()
  }, [authorId, authorProfile?.id, initialAuthor, userId, userRole])
